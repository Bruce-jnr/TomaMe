...
function databaseUrlFromLegacyVariables(env: NodeJS.ProcessEnv): string | undefined {
  const { DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD } = env;
  if (![DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD].every(Boolean)) return undefined;
...
const normalizedEnv = {
  ...process.env,
  DATABASE_URL: process.env.DATABASE_URL ?? databaseUrlFromLegacyVariables(process.env),
  SESSION_SECRET: process.env.SESSION_SECRET ?? process.env.JWT_SECRET,
  PAYSTACK_PUBLIC_KEY:
    process.env.PAYSTACK_PUBLIC_KEY ?? process.env.PAYSTACK_LIVE_PUBLIC_KEY,
  PAYSTACK_SECRET_KEY:
    process.env.PAYSTACK_SECRET_KEY ?? process.env.PAYSTACK_LIVE_SECREY_KEY,
